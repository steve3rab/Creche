...
- **Notes** : création, lecture et modification de notes avec catégorie libre et niveau Normal, Important ou Prioritaire. Recherche dans le titre et le contenu, filtres cumulables par catégorie et importance, tris par modification (récente ou ancienne), titre, importance et catégorie, ou par **Pertinence** : un classement par similarité (index vectoriel construit localement, sans service ni compte distant) qui retrouve une note même si elle ne contient qu'une partie des mots recherchés, plutôt que d'exiger leur présence exacte. Les notes apparaissent aussi dans la recherche globale et sont protégées par les sauvegardes et la corbeille.
...
La recherche par pertinence des notes repose sur un index vectoriel (TF-IDF, similarité cosinus) construit entièrement dans le navigateur, sans réseau ni service externe. Il est mémoïsé par liste de notes : reconstruit une seule fois après un rafraîchissement des données, jamais à chaque frappe. Testé avec 1 000 notes proches de la taille maximale (30 000 caractères, soit plus de 1 000 lignes chacune), la construction de l’index prend environ 1,2 s et chaque requête ensuite moins d’une milliseconde ; avec 200 notes de cette taille, l’index se construit en environ 0,25 s.
