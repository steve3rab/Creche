...
- **Planning** : emploi du temps des créneaux de garde, chacun associé à un membre responsable, avec heure de début et de fin. Navigation mois par mois, affiché en grille mensuelle complète ; un créneau s’ouvre en cliquant dessus pour être modifié ou supprimé (récupérable depuis la corbeille). Un créneau peut se **répéter chaque semaine** jusqu’à une date choisie (deux ans au maximum) : la même personne et le même horaire couvrent alors toutes les semaines concernées en une seule saisie. Modifier ou supprimer une occurrence d’une répétition propose ensuite le choix entre « cette occurrence seulement » et « cette occurrence et toutes les suivantes », pour ajuster une exception ponctuelle sans toucher au reste de l’année ou, au contraire, répercuter un changement durable sur toutes les semaines à venir. Le bouton **PDF** de la barre du mois télécharge le planning du mois affiché (un créneau par ligne, regroupé par jour), généré à la demande sans être conservé sur le disque.
...
- **Notes** : création, lecture et modification de notes avec catégorie libre et niveau Normal, Important ou Prioritaire. Recherche dans le titre et le contenu, filtres cumulables par catégorie et importance, tris par modification (récente ou ancienne), titre, importance et catégorie, ou par **Pertinence** : un classement hybride, entièrement local, qui combine un score lexical (BM25, la formule utilisée par défaut par Elasticsearch/Lucene) et un score sémantique latent (analyse sémantique latente par décomposition en valeurs singulières, qui rapproche deux notes partageant un thème même sans un seul mot exact en commun). Une correspondance exacte reste toujours mieux classée qu'une simple parenté de sens. Les notes apparaissent aussi dans la recherche globale et sont protégées par les sauvegardes et la corbeille.
...
- **Mode clair / sombre** : le bouton en haut à droite (à côté de Rechercher) bascule l’apparence. Le choix est mémorisé sur cet ordinateur et navigateur ; le mode sombre reste celui par défaut.
...
La recherche par pertinence des notes combine deux signaux, calculés entièrement dans le navigateur, sans réseau ni service externe :

- **BM25** (signal lexical) : chaque mot est pondéré par sa rareté dans l'ensemble des notes et par sa fréquence dans la note, avec une saturation (les répétitions supplémentaires comptent de moins en moins) et une normalisation par la longueur du texte, pour ne pas avantager artificiellement une note simplement plus longue.
- **Analyse sémantique latente (LSA)** (signal sémantique) : une décomposition en valeurs singulières (SVD) de la matrice terme-document, tronquée à un petit nombre de « thèmes » latents, projette notes et requête dans un espace réduit où deux textes peuvent se rapprocher parce qu'ils partagent un contexte corrélé, même sans un seul mot exact en commun. Cette SVD est calculée par itération de sous-espace (une méthode numérique classique, sans dépendance externe) sur la matrice de Gram document-document, jamais matérialisée explicitement — seul le produit matrice-vecteur nécessaire à l'itération est calculé, à partir de la représentation creuse des notes. En dessous de 6 notes, ce signal n'a pas assez de matière statistique pour être fiable et la recherche reste purement lexicale.

Le score final privilégie la correspondance lexicale quand elle existe ; le signal sémantique agit comme filet de récupération, avec un seuil minimal pour qu'une note sans aucun mot commun avec la requête ne soit proposée que si sa parenté de sens est nette (évite qu'une similarité résiduelle de fond ne fasse remonter tout le corpus à chaque recherche).

Le résultat (index BM25 + LSA) est mémoïsé par liste de notes : reconstruit une seule fois après un rafraîchissement des données, jamais à chaque frappe — seule la requête est ensuite comparée à cet index, en une fraction de milliseconde. Testé avec 1 000 notes proches de la taille maximale (30 000 caractères, soit plus de 1 000 lignes chacune), la construction de l'index prend environ 2,1 s et chaque requête ensuite quelques millisecondes ; avec 200 notes de cette taille, l'index se construit en environ 0,5 s ; en usage réel (quelques dizaines de notes), en quelques dizaines de millisecondes.
